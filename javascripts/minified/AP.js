...
function AP_UI(bldgInfo){if(!Modernizr.input.placeholder){$("#AP_airshipName").val("Airship Name").unbind("focus").focus(function(){if($(this).val()=="Airship Name")$(this).val("");}).unbind("blur").blur(function(){if($(this).val()=="")$(this).val("Airship Name");});}
...
+"</span></div></div>");}}});$("#AP_buildAirship").unbind("click").click(function(){if($("#AP_airshipName").val()!=""&&$("#AP_airshipName").val()!="Airship Name"){$("#AP_error").val("");var buildAirship=new make_AJAX();buildAirship.callback=function(response){if(response.match(/false/)){$("#AP_error").val(response.split(":")[1]);}else{load_player(player.league,true,true);}};buildAirship.get("reqtype=command&command="+player.command+".createAirship("+$("#AP_airshipName").val()+","+player.curtown.townID+");");}});}
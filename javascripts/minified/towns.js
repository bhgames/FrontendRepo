...
function show_town(){currUI=show_town;$("#window").contents().unbind();$.each(player.curtown.bldg,function(i,v){if(v.update){load_player(player.league,true,true);return false;}else{var noUpdate=true;$.each(v.Queue,function(j,w){if(w.update){load_player(player.league,true,true);noUpdate=false;return false;}});return noUpdate;}});get_buildable();$("#cityname").html(function(){if(player.curtown.townID==player.capitaltid){return"&#171;"+player.curtown.townName+"&#187;";}else{return player.curtown.townName;}});var HTML=" <div id='town_buildBldgMenu'>\
     ...
     <div id='townview'>\
      ...
      </div>";if(player.curtown.zeppelin){HTML+="<img src='AIFrames/buildings/AirshipView.png' id='townback' alt=''/><img src='AIFrames/buildings/enginespin.gif' id='enginespin1' alt=''/><img src='AIFrames/buildings/enginespin.gif' id='enginespin2' alt=''/>";$.each(player.curtown.bldg,function(i,v){HTML+="<div id='pos"+i+"' class='airship bldg'><img src='AIFrames/buildings/Air"+v.path+".png' id='pos"+i+"_building' alt='"+v.type+"' title='Level "+v.lvl+" "+v.type+"'/></div>"});}else{HTML+="<img src='AIFrames/buildings/Base";var numLotsOpen=player.research.lotTech;if(player.capitaltid==player.curtown.townID)numLotsOpen+=4;var townNum=Math.floor((numLotsOpen+1)/5);if(townNum>3)townNum=3;HTML+=townNum+".png' id='townback' alt=''/><div id='pos0' class='bldg'><img src='AIFrames/buildings/MetalMine.png' id='pos0_building' alt='Metal Mine'/></div><div id='pos1' class='bldg'><img src='AIFrames/buildings/TimberField.png' id='pos1_building' alt='Timber Field'/></div><div id='pos2' class='bldg'><img src='AIFrames/buildings/ManufacturedMaterialsPlant.png' id='pos2_building' alt='Manufactured Materials Plant'/></div><div id='pos3' class='bldg'><img src='AIFrames/buildings/FoodFarm.png' id='pos3_building' alt='Food Farm'/></div>";for(i=4;i<=18;i++){HTML+="<div id='pos"+i+"' class='emptylot notMine "+((i>numLotsOpen)?"locked ":"")
+"bldg'><img src='../../images/trans.gif' id='pos"+i+"_building' alt=''/></div>";}}
$("#window").html(HTML+"</div>").fadeIn("fast");if(BUI.CY.bldgServer.length>0){clearInterval(player.townUpdate);player.townUpdate=update_bldg_timers();$("#town_bldgBldgsBar").css("display","block");$("#town_bldgBldgsPopup").unbind("mouseenter").mouseenter(function(){$("#town_bldgBldgsList").stop(true).slideDown();}).unbind("mouseleave").mouseleave(function(){$("#town_bldgBldgsList").stop(true).slideUp();});var list="<ul>";$.each(BUI.CY.bldgServer,function(i,x){var ticksTotal=0;$.each(x.ticksToFinishTotal,function(j,y){ticksTotal+=y;list+="<li><div class='cancelButton noCancel'><a href='javascript:;'></a></div><div class='bldgName'>"
...
if(!player.curtown.zeppelin){$.each(player.curtown.bldg,function(i,x){var lot=x.lotNum;$("#pos"+lot+"_building").attr("title","Level "+x.lvl+" "+x.type);if(lot>3){var back="AIFrames/buildings/"+x.path+".png";$("#pos"+lot).removeClass("emptylot locked").addClass("buildlot");$("#pos"+lot+"_building").attr({"src":back,"alt":x.type});}else if(player.league){$("#pos"+lot+"_building").parent().css("display","none");}
...
ticks=ticksTotal-(x.ticksToFinish+player.time.timeFromNow(1000));}else{ticks=x.ticksToFinishTotal[0]-(x.ticksToFinish+player.time.timeFromNow(1000));$(v).siblings(".cancelButton").removeClass('noCancel');}
if(ticks<1){$(v).siblings(".bldgTicksToFinish").html("updating");load_player(player.league,true,true);}else{var days=Math.floor((ticks/3600)/24);var hours=Math.floor((ticks/3600)%24);var mins=Math.floor((ticks%3600)/60);var secs=Math.floor((ticks%3600)%60);$(v).siblings(".bldgTicksToFinish").html(((days)?days+" d ":"")+hours.toTime()+":"+mins.toTime()+":"+secs.toTime());}

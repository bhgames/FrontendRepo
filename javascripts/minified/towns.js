...
function show_town(){currUI=show_town;$("#window").contents().unbind();get_buildable();$("#cityname").html(function(){if(player.curtown.townID==player.capitaltid){return"&#171;"+player.curtown.townName+"&#187;";}else{return player.curtown.townName;}});var HTML=" <div id='town_buildBldgMenu'>\
...
HTML+="<a href='javascript:;'>"+x.townName+"</a><span>"+x.townID+"</span></li>";});HTML+="</ul>\
...
$("#list").css("width",(numCols*140)+"px");list.fadeIn();$("#list a").unbind('click').click(function(){var that=$(this);player.curtown=$.grep(player.towns,function(v){return(that.siblings("span").text()==v.townID);})[0];map.HTML=map.origHTML;$("#townlist").fadeOut();$("body").unbind('click');that.unbind('click');BUI.build();clearInterval(BUI.active.timer);get_buildable();display_res();build_raid_list();show_town($("#window"));$("#citydropdown").unbind('click').click(function(){town_list();});});setTimeout(function(){$("body").unbind('click').click(function(){$("#townlist").fadeOut();$("#list a").unbind('click');$(this).unbind('click');$("#citydropdown").unbind('click').click(function(){town_list();});});},0);}
function update_bldg_timers(){return setInterval(function(){try{var iter=0;var ticksTotal=0;$(".bldgListID").each(function(i,v){$.each(BUI.CY.bldgServer,function(ind,x){if($(v).text()==x.lotNum){var ticks;if(x.lvlUps>1){if(i!=0){if($(v).parent().prev().children(".bldgListID").text()==$(v).text()){if(iter+1!=x.lvlUps){$(v).siblings(".cancelButton").addClass('noCancel');}else{$(v).siblings(".cancelButton").removeClass('noCancel');}
